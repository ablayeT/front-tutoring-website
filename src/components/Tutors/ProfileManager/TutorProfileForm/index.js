...
import { Box, FormControl,TextField, Typography, Stack, FormLabel } from '@mui/material';
import Button from '../../../Buttons/Button'
import profileFormFields from '../ProfilSchema/tutorProfilForm.shema/index';
...
function TutorProfileForm() {
  ...
  return (
    <Box display='flex' paddingTop='80px' justifyContent='center' width='50%' margin='auto'>
    <FormControl component='form' onSubmit={handleSubmit} encType="multipart/form-data" sx={{ gap:'20px', width:'100%'}}>
  ...
  {formData.imageUrl && <p>{formData.imageUrl.name}</p>}
        ...
        {profileFormFields.map((element) => (
          <Stack key={element.name}>
            <FormLabel sx={{ fontSize: '19px', fontWeight: 'bold' }}>{element.label}</FormLabel>
            <TextField
              type={element.type}
              name={element.name}
              id={element.name}
