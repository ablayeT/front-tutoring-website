...
function ProfileInfos({userInfos,profileInfos}) {
  ...
  return (
    <Box  display='flex'   alignSelf='center'  justifyContent='center' flexDirection='column'> 
     ...
     <TextField type='text' border='1px solid #FFA500' padding='10px' width='50%' value={userData.user[field.key]} />
    </Stack>
    ))}
     ...
     {profileFields.map((field) => (
            ...
            <Stack key={field.key}>
              <FormLabel sx={{ fontSize: '19px', fontWeight: 'bold' }}>{field.label}</FormLabel>
              <TextField type='text' border='1px solid #FFA500' padding='10px' width='50%' value={profileData.profile[field.key]} />
            </Stack>
          ))}
      ...
      {isEditing && (
        <Box display='flex'  flexDirection='column' width='50%' margin='auto'  gap='20px' padding='15px' >
          ...
          {profileFields.map((field) => (
            <Stack key={field.key}>
            <FormLabel sx={{ fontSize: '19px', fontWeight: 'bold' }}>{field.label}</FormLabel>
            <TextField type='text' border='1px solid #FFA500' padding='10px' width='50%' value={editedProfileData[field.key]}  onChange={(e) => setEditedProfileData({ ...editedProfileData, [field.key]: e.target.value })}/>
             </Stack>
             ))}
          <Stack display='flex' justifyContent='center'>
